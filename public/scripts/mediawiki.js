...
// kopia z http://en.wikipedia.org/w/index.php?title=MediaWiki:Gadget-ReferenceTooltips.js&oldid=508635914
// preklad: [[User:Bubamara]]
...
window.pg || $(document).ready( function($) {
    ...
    if ( wgCanonicalNamespace === '' || wgCanonicalNamespace === 'Project' || wgCanonicalNamespace === 'Help' ) {
        ...
        if( settings[0] == 0 ) {
            ...
            footer.append($("<li>").append($("<a>").text("Zapnúť referenčné informácie v bubline").attr("href","javascript:(function(){})()").click(function(){toggleRT(1)})));
            return;
        }
            ...
            function openSettingsMenu(){
                    if( settingsMenu ) {
                            ...
                            settingsMenu = $("<form>").append(
                                    $("<button>").css("width","100%").text("Vypnúť referenčné informácie v bubline").button().click(function(){toggleRT(0)}),
                                    $("<br>"),
                                    $("<small>").text("Po vypnutí možno referenčné informácie v bubline znovu aktivovať kliknutím na odkaz v päte stránky."),
                                    $("<hr>"),
                                    $("<label>").text("Oneskorenie / čas, po ktorom sa objaví bublina (v milisekundách): ").append($("<input>").attr({"type":"number","value":settings[1],step:50,min:0,max:5000})),
                                    $("<br>"),
                                    $("<span>").text("Bublina sa aktivuje:"),
                                    $("<label>").append(
                                            ...
                                            "podržaním kurzora nad referenciou"
                                    ),
                                    $("<label>").append(
                                            $("<input>").attr({"type":"radio", "name":"RTActivate", "checked":settings[2]==1&&"checked"}),
                                            "kliknutím"
                                    )
                            ).submit(function(e){e.preventDefault()}).dialog({modal:true,width:500,title:"Možnosti náhľadu referencií",buttons:{"Uložiť nastavenia":function(){
            ...
            $(this)[ isTouchscreen ? 'click' : 'hover' ](function( e ){
                    ...
                    showTimer = setTimeout( function() {
                            ...
                            if(!tooltipNode){
                                    ...
                                    $( tooltipNode.firstChild.insertBefore( document.createElement( "span" ), tooltipNode.firstChild.firstChild ) ).addClass("RTsettings").attr("title", "Nastavenia referenčných informácií v bubline").click(function(){
